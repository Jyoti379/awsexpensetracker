...
async function addExpense(event) {
    try {
        ...
        await axios.post("http://18.181.172.17:3000/expense/add-expense", obj, { headers: { "Authorization": token } })
            .then((response) => {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    axios.get(`http://18.181.172.17:3000/expense/get-expenses?page=${page}`, { headers: { "Authorization": token} })
        .then(response => {
...
function deleteExpense(expenseId) {
    ...
    axios.delete(`http://18.181.172.17:3000/expense/delete-expense/${expenseId}`, { headers: { "Authorization": token } })
        .then((response) => {
...
function showLeaderboard(){
    ...
    btn.onclick = async (req,res)=>{
     const token = localStorage.getItem('token')
     const userLeaderboard= await axios.get('http://18.181.172.17:3000/premium/showLeaderboard',{headers:{ "Authorization": token }})
...
<td>${userDetails.total_expenses}</td>
 ...
 function download(){
    ...
    axios.get('http://18.181.172.17:3000/user/download', { headers: {"Authorization" : token} })
    .then((response) => {
        console.log(response)
...
document.getElementById('razorpay').onclick = async function (e) {
    ...
    const response = await axios.get('http://18.181.172.17:3000/purchase/purchaseMembership', { headers: { "Authorization": token } })
    console.log(response);
    var options = {
        "key_id": response.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
           const res= await axios.post('http://18.181.172.17:3000/purchase/updateTransactionStatus', {
